...
### Changed
- **Terminology refactoring**: Replaced "proxy" with "websocket/service" throughout codebase
- **Config rename**: `PROXY_PORT` → `WS_PORT`, `ProxyPort` → `WebSocketPort`
- **README restructure**: Configuration moved to top, examples moved to end
- **Test refactoring**: Renamed `TestProxy*` functions to `TestWebSocket*`
